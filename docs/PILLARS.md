...
**How they arrive.** A circle is a group chat before it is anything else, so Kitty meets it there: the self-hosted Telegram bot ([`@KittyCirclesBot`](https://t.me/KittyCirclesBot) in the demo) answers `/circle` and `/score`, pushes every proof and payout, reminds members before a deadline, and opens the dashboard as a Mini App. It greets a saver in their own language (Hindi, Spanish, Swahili, Portuguese, French, Filipino, from Telegram's language code). An organiser opens a circle in one transaction from the browser and sends each signed invite link where the group already talks, with one tap into WhatsApp or Telegram; members consent by joining or by paying once. Payments are ordinary stablecoin transfers on the chain where members already hold money; Creditcoin is where the proof and the credit record live.
