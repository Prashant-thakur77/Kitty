...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    return json(res, 404, { error: 'not found', routes: ['GET /scenarios', 'POST /run/:name', 'GET /status'] });
  } catch (e) {
...
server.listen(PORT, () => {
  ...
  log(`GET /scenarios · POST /run/{${SCENARIOS.map((s) => s.name).join('|')}} (SSE) · GET /status`);
});
