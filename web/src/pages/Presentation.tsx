...
const slides: ReactNode[] = [
  ...
  <Slide key={9} eyebrow="Product" title="Everything a circle needs, from a browser or a phone">
    <ul className="list-disc pl-5"><Li><b>Create</b> a circle in one Creditcoin transaction from <span className="mono">/create</span>: listed members or an open circle with signed invite links; the chain picker reads <span className="mono">get_supported_chains</span> from 0x0FD3 and the start height is placed beyond the attested frontier.</Li><Li><b>Join</b> by invite link (<span className="mono">/join</span>) or accept membership on the circle page; only consented members can be marked missed.</Li><Li><b>Pay</b> on Sepolia and <b>prove</b> the whole round yourself from the same wallet: attestation bounds, Proof Builder, free preflight, one <span className="mono">verifyAndEmit</span> call.</Li><Li><b>Telegram</b>: <a href="https://t.me/KittyCirclesBot">@KittyCirclesBot</a> answers <span className="mono">/circle</span>, <span className="mono">/score</span> and <span className="mono">/steward</span> from chain reads, pushes every proof and payout, reminds before deadlines, and opens the dashboard as a Mini App.</Li><Li>Score dial, rotation wheel, steward decision log and the recorded attack lab, all reading the live ledger.</Li></ul></Slide>,
  <Slide key={10} eyebrow="Live on testnet" title="The first circle has already settled" dense wide>
