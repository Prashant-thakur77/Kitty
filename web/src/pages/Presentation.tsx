...
const slides: ReactNode[] = [
  ...
  <Slide key={2} eyebrow="Trust today" title="Who you have to trust">
    <table><thead><tr><th>Step</th><th>Informal circle</th><th>“ROSCA on-chain” apps</th><th>Kitty</th></tr></thead><tbody>
      ...
      <tr><td>Picks the recipient</td><td>Treasurer / lottery</td><td>VRF oracle</td><td>Deterministic rotation</td></tr>
  ...
  <Slide key={5} eyebrow="Attestcoin depth" title="What the ledger actually checks">
    <ul className="list-disc pl-5"><Li>Batch <span className="mono">verifyAndEmit</span>: up to 10 txs, one continuity proof, one call per round.</Li><Li>EvmV1Decoder: receipt status 1, exactly one Contributed log, emitter = registered vault, tx.to = vault, tx.from = member, exact amount, current round.</Li><Li>Query id = keccak(chainKey ‖ height ‖ txIndex), same as ASCBase; chain key pinned.</Li><Li>Deadlines from attested height: <span className="mono">is_height_attested(chainKey, deadline)</span>.</Li></ul></Slide>,
  <Slide key={6} eyebrow="Attack lab" title="Five ways to cheat, five decoded rejections">
    <ul className="list-disc pl-5"><Li>Replay → QueryAlreadyProcessed</Li><Li>Fake vault with a perfect event → WrongEmitter</Li><Li>Same proof, chain key 3 → WrongChain</Li><Li>Included but reverted tx → SourceTxFailed</Li><Li>Paid after the deadline → accepted, flagged late, −20</Li></ul></Slide>,
  <Slide key={7} eyebrow="Kitty Score" title="Credit history a Creditcoin lender can underwrite">
    <p>500 base · +15 on time · −20 late · −120 missed · 300–850. Every input is a proven transaction or an attested deadline. This is the data Creditcoin was built to carry.</p></Slide>,
  <Slide key={8} eyebrow="Roadmap · CEIP" title="From circles to credit lines">
    <ul className="list-disc pl-5"><Li>Score-gated circle sizes and seat bidding.</Li><Li>ERC-5192 Kitty Score badge; lender integrations on Creditcoin.</Li><Li>Attestcoin writability for payouts once audited; Ethereum mainnet chainKey on CC3 mainnet.</Li><Li>Pot cover through proven-event insurance.</Li></ul></Slide>,
  <Slide key={9} eyebrow="Team" title="Prashant · solo builder">
    <p>Foundry · TypeScript · React. Repo: {cfg.repo}. Contracts on Sepolia and Creditcoin CC3 Testnet; 27+ Foundry tests; local two-anvil rehearsal; live dashboard.</p></Slide>,
]
