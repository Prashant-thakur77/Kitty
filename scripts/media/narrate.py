...
segments = [
 ...
 ("12_depth", "Under the hood, Kitty leans on five ChainInfo functions on the hot path, from the attestation clock to the dashboard asking which attestation covers a payment. It batches verification across circles under one continuity proof, preflights for free through the verifier's view overload, and validates per circle chain keys against the on chain registry. Before any of this was deployed, a real Sepolia proof was verified true by the live precompile on Creditcoin testnet, and a tampered one was rejected."),
