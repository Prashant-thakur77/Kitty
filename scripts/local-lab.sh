...
# every attack scenario is run once and recorded (pnpm lab:record) and the lab API (pnpm lab:api, :8790) is kept
...
echo "== attack lab (every scenario once; the run is recorded to web/public/lab-recorded.json)"
pnpm -s lab:record || echo "!! some scenarios failed (see above)"
